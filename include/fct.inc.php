...
function valideInfosFrais($dateFrais,$libelle,$montant,&$tabErr){
	if($dateFrais==""){
		ajouterErreur("Le champ date ne doit pas être vide", $tabErr);
	}
	else{
		if(!estDatevalide($dateFrais)){
			ajouterErreur("Date invalide", $tabErr);
		}	
		else{
			if(estDateDepassee($dateFrais)){
				ajouterErreur("date d'enregistrement du frais dépassé, plus de 1 an", $tabErr);
			}			
		}
	}
	if($libelle == ""){
		ajouterErreur("Le champ description ne peut pas être vide", $tabErr);
	}
	if($montant == ""){
		ajouterErreur("Le champ montant ne peut pas être vide", $tabErr);
	}
	else
		if( !is_numeric($montant) ){
			ajouterErreur("Le champ montant doit être numérique", $tabErr);
		}
}
/** 
